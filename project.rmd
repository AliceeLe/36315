...
ggplot(shopping_data, aes(x = Age, y = Purchase.Amount..USD., color = Season)) +
  geom_point(alpha = 0.2, size = 0.7) +
  labs(
    title = "Scatterplot - Iris: Purchase Amount vs Age",
    x = "Age",
    y = "Purchase Amount (USD)"
  ) 
```
...
cluster_df <- shopping_data %>%
  mutate(
    discount_bin = if_else(Discount.Applied == "Yes", 1, 0),
    promo_bin    = if_else(Promo.Code.Used == "Yes", 1, 0),
    sub_bin      = if_else(Subscription.Status == "Yes", 1, 0)
  ) %>%
  select(
    Age,
    Purchase.Amount..USD.,
...
aggregate(cluster_df, by = list(cluster = shopping_data$cluster), FUN = mean)


ggplot(shopping_data, aes(x = Age, y = Purchase.Amount..USD., color = cluster)) +
  geom_point(alpha = 0.6) +
  labs(title = "Shopper Clusters", color = "Cluster")
